...
//链式队列:要求在不破坏队列原顺序的前提下,利用两个栈实现队列的逆序输出
...
void reprint_link_stack(link_queue_t *link_queue)
{
    int len_queue;
    int top1 = -1;
    int top2 = -1;

    get_len_link_queue(link_queue, &len_queue);     //获取队列长度
    
    //临时栈1
    int *stack1 = malloc(sizeof(int) * len_queue);

    //临时栈2
    int *stack2 = malloc(sizeof(int) * len_queue);
...
//    }
    ...
    while(top2 >= 0)
    {
        in_link_queue(link_queue, stack2[top2--]);
    }
#endif

    printf("after_reprint:");
...
int main()
{
    ...
    printf("4.1 reprint_link_stack:");
    reprint_link_stack(link_queue);
    printf("\n");
